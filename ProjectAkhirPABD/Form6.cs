    ...
    public partial class Lapangan : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    string LapanganId = dataGridView1.SelectedRows[0].Cells["lapangan_id"].Value.ToString();
                    ...
                    try
                    {
                        using (SqlConnection koneksi = new SqlConnection(stringConnection))
                        {
                            ...
                            // Hapus baris dari tabel "Lapangan"
                            string deleteQuery = "DELETE FROM Lapangan WHERE lapangan_id = @lapangan_id";
                            using (SqlCommand deleteCmd = new SqlCommand(deleteQuery, koneksi))
                            {
                                deleteCmd.Parameters.AddWithValue("@lapangan_id", LapanganId);
